#!/usr/bin/env python3
"""
Raspberry Pi AI Camera를 사용한 얼굴 감지 + 생체신호 측정 + MQTT 전송 시스템
라즈베리파이 5 호환 버전 (서보 모터 제어 제거)
"""
...
class AIBiometricsCamera:
    """AI 카메라를 사용한 얼굴 감지 + 생체신호 측정 + MQTT 전송 (서보 제거 버전)"""
    ...
    def __init__(self, pir_pin=17, start_off=False, 
        ...
        print("\n" + "="*70)
        print("🏥 Raspberry Pi AI Camera Biometrics System (No Servo)")
        print("="*70)
        print("Features:")
    ...
    def draw_overlay(self, frame, face_bbox=None):
        ...
        # 조작키 안내
        cv2.putText(frame, "'q'=quit, 'd'=debug, 'a'=AI, 'm'=MQTT", 
    ...
    def run(self):
        """메인 실행 루프"""
        print("🚀 Starting AI Camera Biometrics System (No Servo)...")
        ...
        try:
            while True:
                ...
                if not self.camera_active:
                    ...
                    cv2.imshow('AI Camera Biometrics System (No Servo)', blank_frame)
                ...
                # 화면 표시
                cv2.imshow('AI Camera Biometrics System (No Servo)', frame)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='AI Camera Biometrics System (No Servo) - Pi 5 Compatible')
    ...
    try:
        # 시스템 정보 출력
        print("🏥 AI Camera Biometrics System (No Servo)")
        ...
        # 메인 시스템 실행
        biometrics_system = AIBiometricsCamera(
