...
class FaceTrackerWithRPPG(FaceTracker):
    ...
    def calculate_and_print_averages(self):
        ...
        if current_time - self.last_avg_time >= 5.0:
            ...
            # 터미널에 출력
            print("\n" + "="*50)
            print(f"[{time.strftime('%H:%M:%S')}] 5초 평균 생체 신호")
            print("="*50)
            if avg_hr > 0:
                print(f"심박수:     {avg_hr:.1f} BPM")
            else:
                print("심박수:     측정 중...")
            
            if avg_stress > 0:
                print(f"스트레스:   {avg_stress:.0f}%")
            else:
                print("스트레스:   측정 중...")
            
            if avg_spo2 > 0:
                print(f"산소포화도: {avg_spo2:.1f}%")
            else:
                print("산소포화도: 측정 중...")
            
            # 디버깅 정보
            if self.spo2_debug:
                print(f"[디버그] HR 유효값: {len(valid_hr)}, Stress 유효값: {len(valid_stress)}, SpO2 유효값: {len(valid_spo2)}")
